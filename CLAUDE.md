...
| Rule | Enforced in |
|---|---|
| One attempt **per quiz** | `UNIQUE(quiz_attempts.profile_id, quiz_id)` + `start_quiz_attempt()` |
| A quiz needs an **open session** | `session_state` on `quizzes` + `start_quiz_attempt()` |
| Scoring and eligibility | `submit_quiz()` / `recompute_final_result()` in `0009_quiz_functions.sql` |
| Answer key confidentiality | `public.public_questions` view + column grants in `0010_rls_four_quiz.sql` |
| Graded work is immutable | `freeze_completed_attempt` / `freeze_answers` triggers |
| Where a signed-in student belongs | `src/lib/auth/route.ts` → always `/dashboard` |
| Admin authorisation | `requireAdmin()` / `requireAdminApi()`, re-run every request |

## Workshop shape

Four quizzes of ten — HTML → CSS → JavaScript → Python — **40 questions, 28 to qualify (70%)**.

The instructor opens and closes each session from `/admin`. A student may attempt a quiz when
its session is **open** and they have no attempt for it. There is deliberately **no**
"must have completed the previous quiz" rule: the session control already sequences the room,
and a per-student rule would permanently lock out anyone arriving late. A missed quiz scores
zero, and the student can still reach 28/40.

The final result unlocks when **every session is closed**, not when a student has done all
four — otherwise a latecomer would never see a result.

Never move a decision from the database into a component. `npm run verify:db` proves these
still hold.

Global frontend standards live in `~/.claude/CLAUDE.md`. This file adds the project's
